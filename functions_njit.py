...
def calculate_retirement_payouts(par, h, s, e, r, t):
    ...
    if (e == 2) or (t >= par.first_retirement and h == 0.0):
        if t >= r + par.m:
            s_retirement = s
            s_lr =  (((1+par.r_s[t])**par.EL[int(r)])*s_retirement*par.share_lr)/np.sum((1+par.r_s[t])**(np.arange(par.EL[int(r)])))
            return  s_lr, 0.0
        
        elif t >= r:
            s_retirement = s
            s_lr =  (((1+par.r_s[t])**par.EL[int(r)])*s_retirement*par.share_lr)/np.sum((1+par.r_s[t])**(np.arange(par.EL[int(r)])))
            s_rp = (((1+par.r_s[t])**par.m)*s_retirement*(1-par.share_lr))/np.sum((1+par.r_s[t])**(np.arange(par.m)))
            return   s_lr, s_rp
...
def value_function(par, sol_V, sol_EV, c, h, a, s, k, e, r, t):
    ...
    # Next period states
    a_next = (1+par.r_a)*(a + income - c)
    s_next = (1+par.r_s[t])*(s + retirement_contribution)
        ...
        if t < par.first_retirement:
            for i in prange(par.simN):
                ...
                if sim_e[i,t] == 2.0:
                    ...
                    if sim_ex[i,t] == 1.0:
                        sim_c[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_c[t,:,:,:,int(retirement_age[i]), int(sim_ex[i,t])], sim_a[i,t], s_retirement[i], sim_k[i,t])
                        sim_h[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_h[t,:,:,:,int(retirement_age[i]), int(sim_ex[i,t])], sim_a[i,t], s_retirement[i], sim_k[i,t])
                ...
                sim_s[i,t+1] = np.minimum(np.maximum((sim_s[i,t] + sim_s_retirement_contrib[i,t] - (sim_s_lr_init[i] + sim_s_rp_init[i]))*(1+par.r_s[t]), 0), par.s_max)
