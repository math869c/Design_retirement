                    ...
                    for s_idx in range(len(par.s_grid)):
                        ...
                        for k_idx in range(len(par.k_grid[t])):
                            ...
                            if t == par.T - 1: # Last period
                                income, _ = final_income_and_retirement_contri(par, assets, savings, human_capital_unemp, hours_unemp, retirement_age, t)
                                ...
                                if employed == int(0.0): # Forced unemployment
                                    ...
                                    income, _ = final_income_and_retirement_contri(par, assets, savings, human_capital, hours_unemp, retirement_age, t)
                                ...
                                if employed == 1.0: # Can choose between employment and unemployment
                                    ...
                                    income, _ = final_income_and_retirement_contri(par, assets, savings, human_capital, h_star, par.last_retirement, t)
                                    cash_on_hand = assets + income
                                ...
                                if employed == int(0.0): # Forced unemployment
                                    ...
                                    income, _ = final_income_and_retirement_contri(par, assets, savings, human_capital, hours_unemp, par.last_retirement, t)
                                    cash_on_hand_un = assets + income
        ...
        if t < par.first_retirement:
            for i in prange(par.simN):
                ...
                if sim_ex[i,t] == 0.0:
                    ...
                    sim_c[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_c[t,:,:,:,int(retirement_age_idx[i]), int(sim_ex[i,t])], sim_a[i,t], sim_s[i,t], sim_k[i,t])
                    sim_h[i,t] = 0.0

                if sim_ex[i,t] == 1.0:
                    # 2. Interpolation of choice variables
                    sim_c[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_c[t,:,:,:,int(retirement_age_idx[i]), int(sim_ex[i,t])], sim_a[i,t], sim_s[i,t], sim_k[i,t])
                    sim_h[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_h[t,:,:,:,int(retirement_age_idx[i]), int(sim_ex[i,t])], sim_a[i,t], sim_s[i,t], sim_k[i,t])
            ...
            for i in prange(par.simN):
                ...
                if sim_ex[i,t] == 0.0 or t == par.last_retirement:
                    if (sim_ex[i,t] == 0.0 and sim_ex[i,t-1] == 1.0) or (sim_ex[i,t-1] == 1.0 and t == par.last_retirement) or (t == par.first_retirement and sim_ex[i,t] == 0.0): 
                        ...
                        # 2. Interpolation of choice variables
                        sim_c[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_c[t,:,:,:,int(retirement_age_idx[i]),int(sim_ex[i,t])], sim_a[i,t], s_retirement[i], sim_k[i,t])
                        sim_h[i,t] = 0.0
                    ...
                    # 1.1 retirement age
                    # 2. Interpolation of choice variables
                    sim_c[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_c[t,:,:,:,t-par.first_retirement, int(sim_ex[i,t])], sim_a[i,t], sim_s[i,t], sim_k[i,t])
                    sim_h[i,t] = interp_3d(par.a_grid, par.s_grid, par.k_grid[t], sol_h[t,:,:,:,t-par.first_retirement, int(sim_ex[i,t])], sim_a[i,t], sim_s[i,t], sim_k[i,t])
